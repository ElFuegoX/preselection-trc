...
def main():
    try:
        ...
        # Auto-refresh personnalisé si activé
        if auto_refresh and st.session_state.running:
            # Créer un placeholder pour l'auto-refresh
            refresh_placeholder = st.empty()
            with refresh_placeholder.container():
                st.write(f"🔄 Auto-refresh actif (toutes les {refresh_interval}s)")
            
            # Auto-refresh avec timer JavaScript
            st.markdown(f"""
            <script>
                setTimeout(function(){{
